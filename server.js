...
function saveListings() {
    fs.writeFileSync(LISTINGS_FILE, JSON.stringify(listings, null, 
2));
}
...
'$2b$10$H0XYgFZuLhE/drO0S3vSVupID5VP5EakmtpmFxIu20znlXQRkXmcS';
...
app.post('/api/admin/listings', verifyToken, upload.single('image'), (req, res) => {
    try {
        ...
        const newListing = { id: Date.now().toString(), section, title, description, priceOrValue, 
imageUrl };
...
app.put('/api/admin/listings/:id', verifyToken, 
...
app.get('/api/listings/:section', (req, res) => {
    const filtered = listings.filter(l => l.section === 
req.params.section);
    res.json(filtered);
});
